"""
VELOCITY WELSH - Unified Social Media Upload Script
"""
...
def get_latest_reel():
    ...
    return {"video_path": str(latest), "metadata": meta, "category": meta.get("category_english", "Welsh Learning"), "phrases": meta.get("phrases", [])}


def generate_caption(phrases, category, platform="facebook"):
    base = [f"Learn Welsh with VELOCITY WELSH!", "", f"Category: {category}", "", f"Master Welsh one phrase at a time! Today's {category} lesson:", ""]
    for i, p in enumerate(phrases[:5], 0):
        base.append(f"{i+1}. {p['english']}")
        base.append(f"   {p.get('welsh', '')}")
        base.append(f"   [{p.get('transliteration', '')}]")
        base.append("")
    base.extend(["Tip: Repeat each phrase out loud 3 times!", "Like this video if you learned something new!", "Comment your favorite phrase below!", f"Follow for daily Welsh lessons!", ""])
    base.extend([f"#learnwelsh", f"#welshlessons", f"#welshforbeginners", "#languagelearning", f"#welshvocabulary", f"#velocitywelsh", f"#dailywelsh", f"#welsh", "#learnlanguages"])
    return "\n".join(base)
        ...
        if func:
            try:
                if pname == "youtube":
                    ...
                    r = func(video_path=video_path, description=caption, title=f"Welsh: {category}")
                elif pname == "telegram":
...
def main():
    print("\n" + "="*80)
    print(f"VELOCITY WELSH - AUTOMATED UPLOAD")
    print("="*80)
